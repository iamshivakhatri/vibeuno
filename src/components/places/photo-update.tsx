...
export function PhotoUpdate({
  ...
  const uploadFiles = async () => {
    ...
    try {
      // Step 1: Request image upload presigned URLs from your API
      const response = await fetch('/api/update-place', {
        method: 'POST',
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      // Step 2: Upload files directly to S3 using the presigned URL
      const uploadPromises = files.map(async (file, index) => {
        const optimizedFile = await optimizeImage(file);
        const uploadResponse = await fetch(presignedData[index].presignedUrl, {
          method: "PUT",
        ...
        setProgress((prev) => prev + (100 / files.length)); // Update progress
        return presignedData[index].presignedUrl; // Return the uploaded file URL
      });
      ...
      toast({ title: "Failed to update photos" });
    } finally {
      setUploading(false);
      setProgress(0);
    }
