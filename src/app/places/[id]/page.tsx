...
import {  getProfileFromClerk, getUserById } from '@/actions/user';
...
export default async function PlaceDetailPage({ params }: { params: { id: string } }) {
  ...
  const loggedinUserId = await getProfileFromClerk(clerkId as string);


  const userId = placeData?.user?.id;

  const profileUser = await getUserById(userId);
  console.log("logged in user",loggedinUserId.userId);
  ...
  return (
    <>
  <PlaceDetail place={place} visitors={visitors} userId={userId} isCurrentUser={isCurrentUser} loggedinUserId={loggedinUserId.userId} />
